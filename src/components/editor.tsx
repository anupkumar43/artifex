...
//   const drawFg = (ctx: CanvasRenderingContext2D) => {
//     if (!fgImage) return;
//     const fg = new Image();
//     fg.onload = () =>
...
//           // ✅ Step 3: revalidate dashboard after upload
//           await refresh();
//           console.log("File uploaded & dashboard refreshed.");
//         } catch (error) {
...
const Editor = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleDownload = async () => {
    ...
    canvas.toBlob((blob) => {
      if (!blob) return;
      void uploadToS3(blob); // 👈 use `void` to mark fire-and-forget
    }, "image/png");
  };
